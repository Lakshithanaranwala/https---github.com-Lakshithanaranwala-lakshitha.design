const DEFAULT_DATA_PATH = 'data/case-studies.json';
...
const getConfig = () => ({
  ...
  dataPath: process.env.GITHUB_DATA_PATH || DEFAULT_DATA_PATH,
});
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (response.status === 404) {
        res.status(200).json({ caseData: {}, caseList: [] });
        return;
      }
      ...
      const parsed = decoded ? JSON.parse(decoded) : { caseData: {}, caseList: [] };
  ...
  if (req.method === 'POST') {
    ...
    try {
      const putResponse = await githubRequest(baseUrl, {
        ...
        method: 'PUT',
        body: JSON.stringify({
          message: 'Update case study data',
          content,
          branch,
          ...(sha ? { sha } : {}),
        }),
